...
def test_stop_multi_integration(jp_unix_socket_file, jp_http_port):
    ...
    # Unix socket.
    p2 = subprocess.Popen(
        ['jupyter-server', '--sock=%s' % jp_unix_socket_file]
    )

    # Specified port
    p3 = subprocess.Popen(
        ['jupyter-server', '--no-browser', '--port=%s' % TEST_PORT]
    )
    ...
    assert MSG_TMPL.format(jp_unix_socket_file) in subprocess.check_output(
        ['jupyter-server', 'stop', jp_unix_socket_file]
    ).decode()

    assert MSG_TMPL.format(TEST_PORT) in subprocess.check_output(
        ['jupyter-server', 'stop', TEST_PORT]
    ).decode()
